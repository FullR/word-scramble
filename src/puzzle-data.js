...
export default [
  ...
  puzzle.id = `puzzle-${puzzle.word}`;
  puzzle.word = puzzle.word.trim();
  puzzle.definition = puzzle.definition.trim();
  puzzle.sentence = blankWord(puzzle.sentence, puzzle.word).trim();
  puzzle.maxLetterHints = Math.floor(puzzle.maxLetterHints / 3);
  return puzzle;
});
